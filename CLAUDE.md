...
- **Three stages**: (1) local iteration — continue the current local batch branch (a new batch starts as `codex/local-batch` from `main`); each bounded target ends with its own local commit and validation, never an automatic push, PR, merge, deploy, or version bump; (2) delivery when the user asks — one PR, the mainline delivery gate, merge, verify the deployed channel. The PR's `preview` label decides the channel (ADR 0057): unlabeled merges deploy directly to Production; labeled PRs deploy to Preview on every push and on merge. Label large or risky work and anything someone should click through first; ask when unsure. (3) Promote Preview-accepted work to Production only when the user requests or has authorized it. Track an in-progress batch in the untracked `plan/local-batch.md`.
...
- `@icm/agent-routing` — Agent-local transient RouteGraph → typed-edit expander. ADR 0007: these types never enter the API schema or persisted model; Agent-side scaffolding with no in-repo importers.
