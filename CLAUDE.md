...
CI (`.github/workflows/ci.yml`) plans validation from the changed paths (`scripts/ci-plan.mjs`, `config/validation-gates.json`). The two required PR checks are `Core contracts` (`ci:static`, `ci:unit`, `release:verify`) and `Browser tests` (only the specs mapped to the PR's paths, or a small fallback). A PR whose checks pass merges through the merge queue (`gh pr merge <number>`), which re-runs the same path-planned checks against current `main`, so branches are not updated by hand just because `main` moved. The four-shard full browser audit runs only when started by hand (`gh workflow run ci.yml`); nothing runs on a schedule.
