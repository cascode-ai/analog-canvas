...
4. When compatible MCP is callable, call connect with the Claim, read analog-canvas://reference/quickstart and analog-canvas://reference/authoring, then call get_context. If installation is declined, blocked, or MCP cannot load now, continue immediately with the HTTP Agent Kit: ${kitUrl}
Fetch the Kit and follow its instructions and published OpenAPI to claim this session and read the current circuit. No plugin installation or restart is needed for the HTTP path. For a loopback address, run on the same computer as the editor. If the Claim expires during setup, ask for a new connection code; do not reuse or invent credentials.
