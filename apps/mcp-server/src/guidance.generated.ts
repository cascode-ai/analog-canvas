...
export const agentToolHelp = {
  ...
  simulation_files:
    "Read/update native source; reuse returned revisions. sync downloads complete files: analysisIndex plus roles:[table] selects a CSV; fileIds selects exact files, [] directory only, omitted selectors all files. prepare-plot takes runId, name, panels and copies an editable Python/config pair plus selected CSVs into the local base; run the copy locally with matplotlib. Existing plots are preserved. basePath is optional. download with artifactId saves locally; outputPath overrides it. artifact without outputPath previews. Details: analog-canvas://reference/mcp-simulation.",
  export_file:
