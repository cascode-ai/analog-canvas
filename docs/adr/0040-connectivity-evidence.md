...
2. A **Net Marker claim** is an owner-addressed electrical naming fact. Net
   Labels, VDD/Ground symbols, and Power Rails all use the same
   `name-claim` shape. A claim owns `name`, `scope`, and optional `powerDomain`.
   Explicit equivalence is additional resolver evidence. SPICE source identity
   is provenance for routing guidance only and never joins Base Nets.
...
default to global scope; ordinary Net Labels default to local
scope. AVDD and DVDD remain separate because their names differ, even though
both carry the `vdd` role.

A Cell Pin remains a hierarchy interface declaration, not a second Net naming
API. Its ordered terminal name may differ from the internal Logical-Net name
and never becomes a marker claim merely because the text matches.
