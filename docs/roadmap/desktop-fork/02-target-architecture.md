...
apps/editor/src/
  ...
  services/                       待新增：宿主服务接口及能力工厂（对齐 #1121）
...
这是渐进式抽离，不要求先搬迁全部 `apps/editor`，也不创建通用插件加载器。两个入口必须导向同一个编辑器实现，不能各复制一个 App。`services/` 是与 #1121 对齐的建议命名；正式实现只选一套能力工厂、注入方式和入口，不再并行建设功能重复的 `hosts/` 框架。
...
U-A 先包装现有 Web 实现，核对账号检查、Cloud 初始加载/窗口聚焦刷新、Gallery 与拓扑检查轮询、Agent 会话恢复、模拟、analytics、service worker 的启动路径。能力缺席时不产生订阅或请求；先让 Web 子目标独立通过回归，Desktop 接入后再验收缺席路径。这个次序不提前接受 #1121 的 Web Save 行为扩展。
...
#1121 另提议 Web 支持 file-bound Save 及首次保存时选择 Cloud/文件。这是后续独立产品目标，不能由以上共用接口自动启用。若接纳，先更新 persistence 规格/ADR，并定义浏览器原位写入、取消/失败和下载回退各自的结果；下载请求不等于确认磁盘保存成功。Desktop 正式保存接入前也需更新该宿主的规格/ADR，不能顺带改变 Web 的既有约定。
...
外链由显式宿主策略控制。fork 的 `shell.openExternal` 是交给系统默认浏览器，并非允许编辑器窗口向外导航；严格离线构建仍禁用这种外部交接，帮助入口提供本地内容或禁用说明。未来需要域名白名单时另定产品配置和承诺，不用普通运行时开关解除严格离线限制。测试同时观察应用请求和外部应用启动。
...
- 与 #1121 的能力工厂导入边界和 offline 构建检查对齐；模块图与 `/api/` 字符串检查只能提供辅助证据，不能替代实际产物的请求、导航、外部交接和主进程出口验证。
...
区分 M1 本地功能验收与 M2 可分发桌面版验收。后者承接 [#1119](https://github.com/cascode-ai/analog-canvas/issues/1119) 的安装/解压产物、文件双击启动、Windows 构建和打包产物零外部请求验收；M1 不包含正式安装与文件关联，不能直接作为 #1119 完成证据。正式签名、发布权限等继续单独落实，#1119 本身不要求签名。
...
作者已在[最新回复](https://github.com/cascode-ai/analog-canvas/issues/1003#issuecomment-5842705867)明确不承担桌面长期维护，只承诺提供合成样例、踩坑记录和答疑支持；主项目仍需安排负责人。样例是待交付输入，取得后核验其原始版本、预期语义及扩展字段，不能用真实保密工作图作为默认索取对象。
