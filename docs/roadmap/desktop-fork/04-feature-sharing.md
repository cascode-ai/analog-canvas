...
作者在[2026-09-26 最新回复](https://github.com/cascode-ai/analog-canvas/issues/1003#issuecomment-5842705867)中澄清了此前对迁入顺序的顾虑：`planWireChain(centerline, jumps = [])` 支持空跳线列表，`wire-chain.ts` 不导入跳线派生函数，主要依赖集中在 `page.ts`。因此保留本计划的先后顺序；去除未接纳字段的调用依赖时，仍保留链式线段、可拖拐点、glue 与 round cap。它们解决的重布线、拐点不可拖和描边缺口问题不依赖跳线功能，不能随字段一起删去。源码边界已核对，真实 Visio 行为仍待验收。
...
根据作者最新的使用需求，**跳线、GaN/IGBT/LDMOS 功率器件、Q 属性表单**优先进入下一轮讨论；MOS body、AGND/DGND、逐段点击连线对其属于较低优先级。这个排序用于安排讨论和契约核对，既不提前接受 schema，也不扩张 05 的首批范围。仅完成首批不能宣称已覆盖作者替代 fork 的全部日常工作流。

## 8. 控制持续维护成本的约定

每项暂时仅 Desktop 的功能记录四件事：入口归属、读/写哪些持久化字段、依赖哪组共享接口、由谁维护对应测试。负责人未明确前可做试验，正式发布前应明确责任。

作者已明确不承担桌面模块长期维护，提供合成样例、踩坑记录和答疑；长期负责人须由主项目安排。是否复用代码、如何记录原贡献与谁负责后续维护分别记录，不能由其中一项推定另一项。实际复用目前仍遵守 [05 的贡献处理停点](05-local-merge-plan.md#feedback-alignment)。
