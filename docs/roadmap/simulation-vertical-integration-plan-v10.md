# Analog Canvas 仿真纵向闭环方案 v10

日期：2026-09-04。版本：v10。状态：讨论方案，供 Fable 交接及后续实施评审；D1/D2 已获产品确认，D3 的环境合同、Issue #551 根因修正与先修复后选型顺序已获产品确认，最终 hosted model profile 仍待实测关闭；所列新增工作尚未实现，尚未替换已接受 ADR。

本文是 v10 讨论方案。[v1 原文](simulation-vertical-integration-plan.md)、[v2 原文](simulation-vertical-integration-plan-v2.md)、[v3 原文](simulation-vertical-integration-plan-v3.md)、[v4 原文](simulation-vertical-integration-plan-v4.md)、[v5 原文](simulation-vertical-integration-plan-v5.md)、[v6 原文](simulation-vertical-integration-plan-v6.md)、[v7 原文](simulation-vertical-integration-plan-v7.md)、[v8 原文](simulation-vertical-integration-plan-v8.md) 与 [v9 原文](simulation-vertical-integration-plan-v9.md) 独立保留、不覆盖；当前讨论以 v10 为准，已接受的产品行为仍以现行 spec 和 ADR 为准。
...
v1 的归档提交 `fc54c56f`、v2 的归档提交 `baaba9be`、v3 的归档提交 `f3d51f43` 及原文均保留。
...
v4 只修正信息架构与图示层次，不表示对应实现已经交付。v4 的归档提交为 `d567b6e9`。
...
v5 只新增并展开前两项已确认方向，同时明确搁置 UI 信息架构。它不表示相应 API、MCP、schema 或界面已经实现。v5 的归档提交为 `d2a246c6`。
...
v6 解决 v5 留下的 UI 信息架构问题，并把“设计 Cell 如何变成 DUT、外部 Cell 如何进入 TB”写成完整产品动作。它仍不表示对应实现已经交付。v6 的归档提交为 `70972978`。
...
| 方面 | v8 修订 |
| --- | --- |
| 最新基线 | 对齐 `origin/main` 的 `ad9d2725`：独立 Preview Worker 与真实 ngspice Container 已经存在，signal-killed 或无输出的运行不再误报完成；不再把 v7 编写时的部署缺口当作当前事实 |
| 结果与运行职责 | F3 唯一负责 run 完成语义、正式 OP/AC/TRAN 结果与 parser；F6 负责云 substrate、限制、隔离、rollout 与 promotion mechanics；F7 只执行候选级垂直验收并形成最终 promotion evidence |
| Agent 资源边界 | Simulation Resource 只拥有 `capabilities / prepare / start / read / cancel`；`configure` 仍是 Circuit/Project typed edit，`export` 仍由 File Resource 交付。MCP convenience 可以跨资源编排，但不复制权威或运输协议 |
| 人类预览切片 | 接受可选 F5-0 作为短期云端垂直探针；直接 `/api/simulate`、手写 testbench 与浏览器临时 log parser 不定义正式 F5，F3/F4/F5 到位后必须退役 |
| 工作包完整性 | F1R 的 Cell Symbol/依赖闭包导入与 F1P 的 account-scoped Project Resource 继续作为独立一等工作包；不能被 F1/F4/F5 的任务卡隐式吞掉 |
| 三项待决 | 云验收通道 D1、promotion evidence/receipt D2、SKY130 模型环境 D3 在本版明确悬置，等待产品讨论；v8 只冻结与其无关的接口和责任边界 |

v8 不是对 v7 的静默改写。v7 的 staging、receipt 与 model-pack 方案保留为历史候选；v8 在三项决策完成前统一使用“选定的云验收通道”“选定的 promotion evidence”“选定的模型环境”等中性术语，不假定讨论结论。v7 在当前重放分支上的归档提交为 `ada39601`。

### v9 相对 v8 的决定

| 方面 | v9 决定 |
| --- | --- |
| D1 云验收拓扑 | 只保留 Preview 与 Production 两个 release channel；独立 Preview Worker 是唯一 cloud acceptance channel，不建立或长期维护 private staging |
| Preview 边界 | 页面、静态资源与只读公开内容可以 public + `noindex`；公开页面不等于公开计算。所有会启动 Container、接收作者文件或执行 raw SPICE 的入口都必须经过明确准入、配额、并发和作业隔离 |
| D2 候选身份 | 候选使用完整 commit SHA；Preview 在验收前后都必须报告匹配的运行时 `buildSha`，正式模型环境可用后再同时核对 opaque `environmentFingerprint` |
| D2 最小证据 | 对应完整 SHA 的成功 GitHub Preview workflow run 本身就是 promotion receipt；只有 GitHub 原生记录缺少的运行时身份才放入小型 check output/CI artifact，不建立数据库或产品 schema |
| Production promotion | Production 只从明确 release/tag 或命名完整 SHA 推广，并要求该 SHA 的匹配 Preview evidence；部署后仍验证自己的 build/runtime identity、health 与恢复路径 |
| 保持开放 | D3 的 SKY130 模型环境、corner 集合、simulator/model 配对、能力范围与资源成本仍是产品决定；D1/D2 的接受不把当前 binned probe 自动升级为产品环境 |

v9 接受 ADR 0057 的双通道方向，并把 v8 的 D1/D2 讨论收敛成最小可执行合同。它不冻结大型 receipt interface、build-once、固定人工审批频率或 Cloudflare rollout 命令；这些实现细节只有在满足上述身份与恢复不变量时才由 F6 选择。v8 的开放讨论原样保留为历史记录。

### v10 相对 v9 的修订

| 方面 | v10 修订 |
| --- | --- |
| D3 根因 | 修正“binned 模型无法覆盖 Issue #551”的过早结论。当前 Preview 丢失 SKY130 随附的 ngspice startup/compatibility contract，导致总宽与每指宽被错误解释，也没有把模型 deck 的单位缩放作为环境语义验证；这是先于 model-pack 选型的 P0 环境缺陷 |
| 启动语义 | Hosted profile 必须以受控、可验证的 startup configuration 冻结 `ngbehavior` 与 W/NF bin-width basis，并验证模型 deck 的 effective scale；不能依赖工作目录、系统偶然存在的 `spinit`，也不能把修复暗写进用户 deck |
| Profile 边界 | 接受“可命名仿真环境 Profile”方向，但把数值环境、单次运行身份、qualification evidence 与 deployment policy 分层；不把绝对路径、某次冷启动时间或诊断枚举混成一个巨大电气对象 |
| Binned/continuous | 修复 startup contract 后，以同一 simulator、harness 和 fixture 重新量测 binned；continuous 仍是候选，但不再由 Issue #551 直接推出，也不允许静默 fallback、混用或几何改写 |
| 验收门禁 | 增加 NFET/PFET 多指宽管正例、单指真实越界负例、corner/scale/compatibility 断言与 model-backed Preview gate；纯电阻 platform smoke 不再充当 SKY130 模型证据 |

v10 不改变 v9 已接受的 D1/D2、Cell/Testbench、structured/raw、Agent parity、极简 UI 与 promotion 方向。它只修正 D3 的事实基础，并把已经讨论接受的环境分层和验收顺序写成可执行边界；最终首版公开 binned、continuous 或另一经验证 pack，仍须由修复后的同条件实验关闭。
...
v2 补充讨论时检查的主工作树为 `42f31aa5`。v3 的 Project / Cell / 云端管理定向审查基线为 main `a469fa68`（#548）；v5 对 Agent Project 管理与无图 raw 创作、v6 对 Cell Symbol/TB 衔接与人类 UI、v7 对 Cloudflare staging/deployment 的定向审查也以该 main 状态为依据。v8 的定向审查基线更新为 `origin/main` 的 `ad9d2725`（#566）：独立 public/noindex Preview、真实 Cloudflare ngspice Container、Run Metadata V1/environment fingerprint 与 signal/空输出失败分类已经落地。v9 在同一基线上冻结 D1/D2 的产品方向；v10 又对固定 ngspice 47、当前 Ciel/OpenPDKs binned runtime、PDK `spinit`、ngspice 官方 compatibility 规则和 Issue #551 deck 做了定向复核。PR #559、#563 仍是 open implementation proposal，Issue #560、#561、#564 仍是 task card，产品确认不等于代码已经合入。下列“当前”能力是这次审查记录，实施前仍须重新确认。

目标 owner：Fable。代码实施另开 `codex/` 分支；本文件记录产品讨论结果，但 D1/D2 仍需同步到正式 ADR/spec/deployment 并实现。D3 的合同与修复顺序已确定，最终 hosted profile 仍须在 startup 修复和资格实验后关闭。
...
Promotion 的权威证据是对应完整 commit SHA 的成功 GitHub Preview workflow run，不建立独立数据库或产品 receipt schema。Preview gate 在验收前后都必须从运行 endpoint 读取并确认 `buildSha === expected full SHA`，避免历史绿色记录、旧 Worker 或验收中途覆盖被误认成当前候选。D3 环境合同就绪后，同一 gate 还记录并核对 opaque `environmentFingerprint`；必要时可附一个稳定可取得的 Cloudflare deployment/version identity，但不同时复制多个平台 ID。

Production release/tag 或手动入口必须解析到完整 SHA，并只接受该 SHA 的匹配成功 Preview run；部署后继续校验自己的 build identity、health 和实际 runtime fingerprint。Preview 成功不替代 Production 的轻量 smoke、Container rollout 收敛与独立恢复。是否 build once、允许哪些配置 overlay、是否增加人工审批及具体 rollout 命令不在 v10 写死；若选择重建，实际身份仍必须与已接受环境合同匹配。事故恢复可以使用仓库既有 production-incident 例外，但必须留下书面记录，普通 release 不得静默绕过门禁。

#### D3：SKY130 模型环境

[Issue #551](https://github.com/cascode-ai/analog-canvas/issues/551) 的原始定性不成立：`l=3 w=200 nf=20` 不是对当前 binned pack 的总宽越界证据。SKY130 wrapper 将 `w` 与 `nf` 传给内部 BSIM4；PDK 随附的 ngspice `spinit` 明确设置 `ngbehavior=hsa`，使 bin selection 按每指 `W/nf` 解释宽度；模型 deck 的 `all.spice` 另以 `.option scale=1.0u` 定义无后缀 W/L 的单位缩放。该实例的有效每指宽为 10 µm，落在当前 TT NFET 的 L=2–4 µm、每指 W=7–100 µm model card 内。使用固定 ngspice 47 和相同 binned deck 的对照已复现：native 启动失败；受控 `hsa` startup 成功完成 OP。因此当前缺陷是运行时初始化契约在镜像裁剪/启动边界丢失，不是 exporter、netlist 或 binned 总宽上限。
...
2. `SimulationRunIdentity`：本次 environmentId、corner、temperature、prepared deck hash 与实际 runtime fingerprint；
3. `QualificationRecord`：fixture 集版本、分析类型、基准值与数值容差；
...
| 执行 | Worker route、Docker harness、local-host、60 秒默认/120 秒上限、Preview Worker 的 `standard-2` 单实例 Container、环境 metadata；signal-killed/无输出不再算完成 | 受控 SKY130 startup contract 与能力 Profile；正式 async run/result、鉴权与配额、busy guard、非 root/每作业隔离、进程树终止、原始数值收集与 rollout 收敛证据 |
| 结果 UI | OP 标注函数、AC 图形组件、未挂载的模拟面板原型 | 轻量 task bar、按需设置与结果 drawer；OP/AC/TRAN 正式数值协议、stale 防护、CSV；删除浏览器临时 log parser |
...
5. 环境必须有可验证 lock。数值 Profile 锁定 ngspice binary、模型来源/content hash、受控 startup config 及 compatibility/effective-scale/width-basis；Deployment Policy 锁定基础/runtime image、资源与隔离。实际 runtime 必须同时报告 profileId 和 opaque fingerprint；在首个 Profile 通过 qualification 前只能报告 observed metadata，不能宣称产品环境已经 pinned。
...
- workflow concurrency 串行化正常部署；验收前后都从 endpoint 读取 `buildSha` 并要求匹配 expected full SHA，捕获错误部署、历史版本或 out-of-band 覆盖；Cloudflare deployment/version ID 只有在实测证明必要且稳定时才补一个；
- 通道不可用时允许普通产品继续明确保持 production simulation disabled，但不得启用或更新未完成云验收的 production simulation。

自动验收至少覆盖：

1. Worker shell、capabilities、DO binding/migration、Container health、cold/warm start、实际实例规格、声明 profileId 与 environment fingerprint；缺少或篡改 startup config 必须 fail closed。
...
### 8.4 已接受的 Cloud gate/promotion 合同与待资格关闭的 D3
...
D2 已决定：完整 commit SHA 是 source candidate，endpoint 报告的 `buildSha` 是 deployed candidate，F3/D3 的 opaque `environmentFingerprint` 是 runtime candidate。对应 full SHA 的 required GitHub Preview workflow run 在完整验收成功后就是 promotion receipt；GitHub 已保存 run ID、workflow revision、时间、日志与 conclusion，只有运行时身份需要一个小型 output/artifact。它不进入 Project、SimulationSetup、SimulationResult 或 File Resource，也不产生第四套 metadata 服务。产品 API 的短期 `start receipt/runId` 是一次仿真作业句柄，与这里的发布 receipt 无关。

Preview gate 必须在验收前后确认 `buildSha === expected full SHA`，并记录声明的 profileId 与 opaque `environmentFingerprint`。Production promotion 只接受该 SHA 最新已完成且成功的 required Preview run；较新的 failed/cancelled run 会阻止普通 promotion，直到重新验收成功。Production checkout/deploy 同一 full SHA，并在上线后验证自己的 build identity、health 与实际 runtime fingerprint。v10 不要求 build-once、完整 Worker bundle 字节相同、巨型 manifest/fixture hash 清单或固定人工审批频率；若选择重建，锁定依赖与运行时身份检查必须证明实际环境仍符合合同。
...
D3 的分层合同、startup P0 和资格顺序已经确定；最终首个 hosted Profile 仍须由修复后的 binned 与 continuous 候选实验关闭。在受控 startup、manifest 和 Qualification Record 均完成前，Preview 的现有 binned platform probe 不能被描述为正式产品模型环境，模型相关 F7 gate 与 production simulation 上线仍未完成。
...
### F0：冻结最小产品契约、正式化 D1/D2/D3 并形成首个 Profile 候选

Owner：编排负责人。范围：ADR 0055、simulation spec、现有 hierarchy/project 合同及相关 architecture/user/deployment 文档；不写另一份并行总规范。
...
- D1 的 Preview/Production 双通道、受保护 simulation execution、数据隔离与 legacy staging 退役写入 ADR 0057、simulation spec 和 deployment 文档。
- D2 的 full SHA、endpoint `buildSha` 前后校验、GitHub Preview run receipt、opaque `environmentFingerprint`、exact-SHA promotion 与恢复规则写入 deployment/workflow contract tests；不新增产品 schema。
- D3 把四层合同写入 simulation spec/ADR/deployment：deployment-side Profile manifest、每次 Run Identity、CI Qualification Record 与 F6 Deployment Policy。先交付受控 startup config、profile schema/manifest、资格 fixture/容差定义和第一候选的真实来源/digest；F0 冻结资格标准，F7 才签署实际 Qualification Record。
...
退出：上述产品合同、已决定的 D1/D2/D3 分层与第一候选资格标准在现行 spec/ADR/deployment 文档中明确；`docs/specs/simulation.md` 不再把部署绝对路径当产品身份，ADR 0055 中与 structured OP/AC/TRAN 冲突的旧边界被有意修订；open PR/Issue 的边界被吸收或重新定性。没有改变 DUT/Net、覆盖 raw 文本、复制 setup owner、文件运输或云验收协议的隐含路径；不为 sweep/优化预建通用工作流。
...
Owner：spice-run/result parser；依赖 F2。local-host 与 hosted executor 都消费这一合同，F3 不拥有 Cloudflare binding、实例规格或 promotion。
...
F3 是唯一回答“这次 run 是否形成有效结果”的层。进程正常退出只是必要条件；signal/OOM、timeout、cancel、空或被截断输出、必需 plot/vector 缺失及 parser failure 都必须有稳定分类。Worker、GUI 和 MCP 不得分别从 exit code 或 console 文本再推导一套完成语义；浏览器的临时 log parser 在正式 parser 到位后退役。
...
Owner：apps/editor 的隔离 adapter；只在需要尽快观察真实云链路时实施，不是正式 UI 的前置条件。它可以复用现有未挂载 panel，完成 current document design export → 作者 testbench 文本 → 当前 `/api/simulate` → log/临时 OP 显示；不改 Project schema、不保存结果、不定义正式资源或结果合同。

边界必须写在实现和任务卡里：direct POST 集中在一个可替换 client，临时 OP log parse 集中在一个 adapter；F3 到位后删除浏览器 parser，F4 到位后删除 direct POST，F1/F5 到位后由保存的 SimulationSetup、task bar/drawer 和 typed result 替代手写-only 流程。也可以完全跳过 F5-0，继续使用 CI/API platform probe 等待正式 F3/F4。
...
Owner：worker/containers/deploy workflow。D1/D2 与 D3 分层边界已决定，相关 mechanics 可以立即推进；最终首个 model pack 仍待 qualification。F6 复用现有 Preview 作为唯一验收面，不新建 staging，并实现受保护计算、候选身份、promotion 与恢复边界，不替产品讨论签署电气能力。

F6 负责真实 binding/DO/Container、安装并在解析 deck 前确定加载声明 Profile 的受控 startup config、实际实例规格、单槽/busy、timeout/process-tree kill、非 root 与每作业隔离、输入输出上限、cold/warm、health、runtime fingerprint、rollout convergence 和独立 Container recovery。startup bytes、实际 compatibility/effective-scale/width-basis 必须可验证；语义配置与线程/KLU 等资源选项分开。当前 Preview 的 Worker、Container、`standard-2` 与平台 probe是可复用基础，不重建第二套 route 或 executor。Preview 的 simulation execution 加入 CI service identity、授权测试者准入和配额；公开 raw `.control` 与 structured 输入都按不可信作业处理，不能靠只允许 helper 来规避隔离。release-only Production 稳定后退役 legacy `env.staging`、gate、workflow job 与残留 Worker。

F6 不拥有 run 的数值完成语义和 parser（F3），不拥有 GUI/MCP 闭环（F4/F5），不签署某个 Profile 是否通过（F7），也不决定其电气支持范围（D3/F0）。它注入完整 build SHA、提供只读 build identity、在 Preview gate 前后核对该 identity，记录 profileId + opaque environment fingerprint，并让 Production 只消费 exact SHA 最新已完成且成功的 required Preview run。GitHub run 是 receipt；F6 只在必要时附小型 machine-readable output，不建立 receipt 数据库或产品协议。

退出：Preview 能以受控身份重复运行真实 Container；鉴权/配额、busy、timeout/cancel、进程树、作业隔离和 artifact 留存可验证；实际 runtime 规格、warm/cold、peak RSS、TRAN 输出体积和费用有证据。Preview 验收前后 `buildSha` 匹配 expected full SHA，required run 的 latest-completed 结论控制 exact-SHA promotion；Production 部署后验证 build/runtime identity 与 health。rollout 未收敛时不会公开启用新 simulation，Container target 可独立恢复。关掉 simulation 不影响 Editor、Gallery 或本地文件导出。

### F7：垂直验收与上线

Owner：整合负责人。依赖 F3/F4/F5/F6。

F7 针对一个完整 SHA 候选，在 Preview 执行平台、数值、协议、隔离、GUI/MCP 与跨工程完整矩阵，消费 F3 的正式结果和 F6 的 substrate。所有 required jobs 成功才使该 GitHub Preview workflow run 成为 promotion receipt；F7 不实现第二个 runner、parser、smoke API 或 receipt schema。localhost/local Docker 只作为更早的开发证据；production 不承担首次发现云绑定、Container、模型或结果协议错误的职责。
...
18. Cloud promotion：不能写 production 私有数据且 execution 受保护的 Preview 运行自动平台/OP/AC/TRAN、D3 Profile、raw 和失败门禁，GUI 与 MCP 各完成一条自动 smoke。gate 前后 `buildSha` 都等于 expected full SHA，并记录声明 profileId 与 opaque `environmentFingerprint`；所有 required jobs 成功后该 GitHub run 成为 receipt。Production 只推广同一 SHA 最新已完成且成功的 required run，部署后确认自己的 build/runtime identity、Container rollout 与 health，并保留独立 Worker/Container recovery。v10 不要求固定人工 review 频率。
...
```text
F0
├─ formalize accepted D1 / D2 / D3 layers + first Profile qualification criteria
├─ D3-P0 controlled startup → binned/continuous candidate experiment
├─ F1 / F1R / F1P → F2 → F3
...
F6 的 Preview 鉴权/runtime hardening、build identity 与 exact-SHA workflow gate 可立即并行；D3 只阻塞模型 Profile 相关 acceptance 与最终 production simulation 上线。startup P0 可独立先修，但 Profile 只能在同条件 qualification 后签署。F3 结果合同就绪后形成自动数值/API gate，F4/F5 完成后再加入 MCP/GUI 云闭环；F5-0 可更早用于观察链路，但没有任何正式完成度含义。F2 的源/编排文件导出不等待 F3 跑通；F4 的 transport/workspace 可在 F0 的 RawInputBundle 合同冻结后并行，最终 simulation 接入仍依赖 F2/F3。先得到 Agent 可运行且在 Preview 验证的 OP 垂直切片，再扩展 AC、TRAN、raw 输出与 v10 task UI；三分析、两种输入、中间产物、Current top→TB、按需 UI 与 cloud acceptance 均是首期退出条件，不能在仅 localhost 或 platform probe 完成时宣称 v10 闭环完成。

### 当前 GitHub 承接关系

| 条目 | 在 v10 中的定位 |
| --- | --- |
| [PR #559](https://github.com/cascode-ai/analog-canvas/pull/559) | F0 的 normative contract 候选；合入前更新到 v10，写入已决定的 D1/D2 与 D3 四层合同、startup P0 和资格顺序；最终 Profile 不能用未完成实验的假值占位 |
| [Issue #560](https://github.com/cascode-ai/analog-canvas/issues/560) | F1 task card，方向与 v10 一致 |
...
F1R、F1P、F2、F3 仍需各自建立有明确 owner 和退出条件的 task card。尤其 F1R/F1P 不是 F4/F5 的内部实现细节；若只执行 #560/#561/#564，多 Cell、跨 Project DUT 与 Agent Project management 会从交付中消失。
...
- 第二个 Editor App、Simulation 专用 route、常驻三栏/多宫格 dashboard；v10 只增加同一 Editor 内的临时 task bar、按需交互与结果 drawer。
...
本地 authority pack 已存在：`.reference-src/icm-resources/simulation-authority/`。`manifest.json` 保存下载来源与 hash；有 ngspice46结构基线手册、固定 ngspice47 手册/源码/Windows binary、SkyWater/OpenPDKs/Ciel 模型、PDK `spinit`、参考 benchmark、`.lib` 及 startup/W-NF 对照。它是本机忽略资产，Fable 在其他机器不会自动获得；按 manifest 重建或另行传递，不悄悄提交模型大包。
...
- `.github/workflows/cloudflare.yml`、`.github/workflows/deploy-preview.yml`
- `docs/adr/0057-release-channels-preview-and-production.md`、`docs/deployment.md`
...
- https://nmg.gitlab.io/ngspice-manual/circuitelementsandmodels/generaloptionsandinformation/modelbinning.html
- https://nmg.gitlab.io/ngspice-manual/circuitelementsandmodels/generaloptionsandinformation/parallelingdeviceswithmultiplierm.html
- https://nmg.gitlab.io/ngspice-manual/circuitdescription/lib.html
- https://nmg.gitlab.io/ngspice-manual/circuitdescription/include.html
- https://nmg.gitlab.io/ngspice-manual/analysesandoutputcontrol_batchmode/analyses/tran_transientanalysis.html

v1/v2/v3/v4/v5/v6/v7/v8/v9/v10 编写与归档没有修改产品代码或部署。v10 额外执行了固定 ngspice47 + 当前 binned runtime 的 native/hsa 对照，用于纠正 Issue #551 根因；它仍只是讨论方案和资格证据，不代表 Container 修复已经交付。v1–v9 原文均保留，当前实施讨论使用本文 v10。D1/D2 与 D3 分层/顺序已由产品讨论接受，但只有同步到正式 spec/ADR/deployment、实现受控 startup 并由 F7 资格验证后才成为仓库行为；最终首个 hosted Profile 仍待关闭。open PR/Issue 不因被引用而自动成为已接受 API/schema/deployment 合同。
