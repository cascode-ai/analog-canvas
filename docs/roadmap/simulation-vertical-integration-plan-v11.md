# Analog Canvas 仿真纵向闭环方案 v11

日期：2026-09-04。版本：v11。状态：当前完整讨论方案，供 Fable 交接及后续实施评审；D1/D2 和 D3 的环境合同、根因修正及资格顺序已获产品确认，最终 hosted model profile 仍待实测关闭；新增工作尚未实现，尚未替换已接受 ADR。
...
§8.4 是 promotion 的唯一规范规则，本节只给摘要：F6 先新增并暴露只读 `buildSha`，仿真运行身份复用现有 `metadata.environment.fingerprint`；Preview gate 再核对部署身份、运行环境与完整验收结论。普通 promotion 必须满足 §8.4 的 latest-completed required-run 规则；GitHub run 本身就是 receipt，不建立数据库或产品 receipt schema。Production 部署后仍执行自己的 identity、health、rollout 收敛与恢复验证。
...
1. Worker shell、capabilities、DO binding/migration、Container health、cold/warm start、实际实例规格、声明 profileId 与 `metadata.environment.fingerprint`；缺少或篡改 startup config 必须 fail closed。
...
D2 的唯一规范规则如下。对完整候选 SHA `S`，普通 Production promotion 仅在这些条件全部成立时允许：F6 已把 `S` 注入 Preview runtime 并由只读 identity endpoint 暴露为 `buildSha`；required Preview workflow 在完整验收前后均确认 `buildSha === S`；在 GitHub 上针对 `S` 的所有已完成 required Preview runs 中，按完成时间最新的一次 conclusion 为 success；模型资格 gate 还记录同一 runtime 的 profileId 与现有 `metadata.environment.fingerprint`。较新的 failed/cancelled/timed_out run 阻止普通 promotion，直到更新的成功验收完成；in-progress run 不构成 receipt。Production checkout/deploy 同一 `S`，上线后再次验证 build identity、runtime identity 与 health。
...
D3 的分层合同、startup P0 和资格顺序已经确定；D3-P0 的修复后 binned/continuous 候选实验只提供选择证据，最终首个 hosted Profile 必须由 F7 明确选择并签署 Qualification Record。在受控 startup、manifest 和 Qualification Record 均完成前，Preview 的现有 binned platform probe 不能被描述为正式产品模型环境，模型相关 F7 gate 与 production simulation 上线仍未完成。
...
- D2 对 ADR 0057 的宽松 green-preview 证据做显式 amendment，并把 §8.4 的唯一规则写入 deployment/workflow contract tests：F6 先注入/暴露 `buildSha`，gate 再核对 full SHA、GitHub required run receipt、现有 `metadata.environment.fingerprint`、exact-SHA promotion 与恢复；不新增产品 schema。
...
退出：上述产品合同、D1/D2/D3 分层与首个候选的资格标准在现行 spec/ADR/deployment 文档中明确；`docs/specs/simulation.md` 不再把部署绝对路径当产品身份，ADR 0055 的首版 OP/AC 范围以及 ADR 0057 的匿名 simulation route、宽松 green-preview 证据均被显式修订，`docs/deployment.md` 不再同时保留 Preview Container 与 legacy staging/no-simulation 两套相冲突的现行叙述；open PR/Issue 的边界被吸收或重新定性。F0/编排负责人已经创建并在本文链接 F1R、F1P、F2、F3、D3-E0、D3-P0 六张独立 task card，每张均有 accountable owner、owned paths、依赖、退出条件和验证 gate。没有改变 DUT/Net、覆盖 raw 文本、复制 setup owner、文件运输或云验收协议的隐含路径；不为 sweep/优化预建通用工作流。F0 可在合同与 handoff 完成后关闭，不等待 F7 的最终 Qualification Record。

### D3-E0：可重建的仿真权威资料

Owner：F7 qualification/integration owner；F3 与 F6 是消费者。该目标不把模型大包或 simulator binary 提交进仓库，而是让 ignored authority pack 能从 clean checkout 确定性重建。

交付 tracked bootstrap/verify procedure：按 manifest 的固定 URL/commit 获取资料，逐项校验 size、SHA、license 与 startup evidence，确定性解包所需 runtime，并运行最小 reference decks。仓库保存小型 manifest、脚本、可分发期望值和许可记录；大型模型、手册和二进制继续位于 ignored `.reference-src`。来源/hash 不匹配必须 fail closed，不能靠某台机器手工拷贝才能验收。

退出：在声明的工具与网络条件下，clean checkout 可重建 pack、核对 simulator/model/startup digest，并运行 D3-P0/F7 所需最小 reference harness；失败能指出具体 artifact，而不是退化成未经验证的本地文件。

### D3-P0：受控 startup 与候选 Profile 比较

Owner：F6 的 worker/containers runtime owner。依赖 F0 冻结的 Profile/startup/资格合同，以及 D3-E0 可重建 authority pack；不依赖 F4/F5 UI。

交付仓库拥有、版本化、被 hash、在解析 deck 前确定加载的 startup config，缺失或篡改必须 fail closed。使用完全相同的 ngspice binary、harness、deck、corner、限制与数值比较方法，运行 native binned 失败对照、修复 startup 的 binned、digest-pinned continuous 三候选；记录模型来源/许可/digest、有效 scale、compatibility、W/NF basis、CPU/RSS、cold/warm 和 fixture 结果。D3-P0 不自行签署产品 Profile，也不允许静默 fallback 或几何改写。

退出：受控 startup 与 runtime identity contract test 通过；三候选比较记录可由 F7 在 clean checkout 重放；失败候选保留诊断，不伪装为产品支持。其结果只阻塞 model-backed acceptance 与 production simulation，不阻塞 F1/F2/F3 的非模型合同工作。
...
Owner：worker/containers/deploy workflow。D1/D2 与 D3 分层边界已决定，相关 mechanics 可以立即推进；最终首个 hosted Profile 仍待 qualification。F6 复用现有 Preview 作为唯一验收面，不新建 staging，并实现受保护计算、候选身份、promotion 与恢复边界，不替产品讨论签署电气能力。

F6 负责真实 binding/DO/Container、安装并在解析 deck 前确定加载声明 Profile 的受控 startup config、实际实例规格、单槽/busy、timeout/process-tree kill、非 root 与每作业隔离、输入输出上限、cold/warm、health、runtime fingerprint、rollout convergence 和独立 Container recovery。Deployment Policy 显式锁定 OCI `linux/amd64`，构建/部署不得依赖 runner 默认架构，验收将实际 `linux/x64` 观测规范化后核对；startup bytes、实际 compatibility/effective-scale/width-basis 必须可验证，语义配置与线程/KLU 等资源选项分开。当前 Preview 的 Worker、Container、`standard-2` 与 platform probe 是可复用基础，不重建第二套 route 或 executor。Preview 的 simulation execution 加入 CI service identity、授权测试者准入和配额；公开 raw `.control` 与 structured 输入都按不可信作业处理，不能靠只允许 helper 来规避隔离。release-only Production 稳定后退役 legacy `env.staging`、gate、workflow job 与残留 Worker。

F6 不拥有 run 的数值完成语义和 parser（F3），不拥有 GUI/MCP 闭环（F4/F5），不签署某个 Profile 是否通过（F7），也不决定其电气支持范围（D3/F0）。它按 §8.4 注入完整 build SHA、提供只读 build identity，并让 Preview gate 读取现有 `metadata.environment.fingerprint`；promotion 资格完全复用 §8.4，不在 F6 另定义一套规则。GitHub run 是 receipt；F6 只在必要时附小型 machine-readable output，不建立 receipt 数据库或产品协议。

退出：Preview 能以受控身份重复运行真实 Container；鉴权/配额、busy、timeout/cancel、进程树、作业隔离和 artifact 留存可验证；实际 runtime 规格、OCI platform、warm/cold、peak RSS、TRAN 输出体积和费用有证据。§8.4 的 build/runtime identity、exact-SHA promotion 与 post-deploy identity/health verifier 合同测试全部通过，rollout 未收敛时不会公开启用新 simulation，Container target 可独立恢复。F6 交付可执行的 promotion/recovery mechanics，但不以一次实际 Production simulation promotion 作为自己的前置退出条件；候选的 required Preview receipt、实际 promotion 和上线后证据由 F7/release 流程完成。关掉 simulation 不影响 Editor、Gallery 或本地文件导出。

### F7：垂直验收与上线

Owner：整合负责人。完整上线验收依赖 F0、F1/F1R/F1P、F2-F6、D3-E0 与 D3-P0；D3 Profile 资格切片在 F3、D3-E0、D3-P0 就绪后即可先行，不等待 GUI/MCP。
...
├─ create/accept F1R / F1P / F2 / F3 / D3-E0 / D3-P0 task cards
├─ F1 / F1R / F1P → F2 → F3
...
F3 + hardened Preview + selected Profile
...
F1R、F1P、F2、F3、D3-E0、D3-P0 仍需各自建立有明确 owner 和退出条件的 task card，创建并由 owner 接受这些卡是 F0 的退出门禁。尤其 F1R/F1P 不是 F4/F5 的内部实现细节；若只执行 #560/#561/#564，多 Cell、跨 Project DUT、Agent Project management 与可重建模型资格都会从交付中消失。
...
本地 authority pack 已存在：`.reference-src/icm-resources/simulation-authority/`。`manifest.json` 保存下载来源与 hash；有 ngspice46 结构基线手册、固定 ngspice47 手册/源码/Windows binary、SkyWater/OpenPDKs/Ciel 模型、PDK `spinit`、参考 benchmark、`.lib` 及 startup/W-NF 对照。它是 ignored 工作资产，不是可移植门禁；D3-E0 必须交付 tracked bootstrap/verify procedure，使 clean checkout 能按固定来源重建并 fail-closed 校验。大型资料不因此提交进仓库。
...
- `apps/editor/src/document/use-project-file-lifecycle.ts`
...
- `apps/editor/src/features/hierarchy/`、`apps/editor/src/features/simulation/`
