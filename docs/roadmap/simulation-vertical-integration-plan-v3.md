# Analog Canvas 仿真纵向闭环方案 v3

日期：2026-09-04。版本：v3。状态：讨论方案，供 Fable 交接及后续实施评审；所列新增工作尚未实现，尚未替换已接受 ADR。

本文是 v3 讨论方案。[v1 原文](simulation-vertical-integration-plan.md) 与 [v2 原文](simulation-vertical-integration-plan-v2.md) 独立保留、不覆盖；当前讨论以 v3 为准，已接受的产品行为仍以现行 spec 和 ADR 为准。
...
v1 的归档提交 `9c1960b6`、v2 的归档提交 `96d9f9d1` 及原文均保留。v3 是设计与验收条件的补充，不表示对应实现已经交付。
...
目标 owner：Fable。代码实施另开 `codex/` 分支；本文件是已归档的 v3 讨论方案，不算已接受产品契约。
...
flowchart TD
  ...
  A --> U
  U -->|同工程：直接放置| E
  B[本地文件 / 授权云 Project 快照] --> K[共用 Cell 导入 planner：依赖闭包与引用重映射]
  U -->|跨工程：选择并复制| K
  K --> E
  E --> P[目标 Project：本地 Cell 定义 + 多实例 TB + Setup]
...
F6 的环境/部署验证在 F0 后并行，最终与 F3/F7 汇合。F2 的源/编排文件导出不等待 F3 跑通；F4 可在合同冻结后并行接入。先得到 Agent 可运行的真实 OP 垂直切片，再扩展 AC、TRAN、raw 输出与完整 GUI；三分析、两种输入和中间产物均是首期退出条件，不能在仅 OP 完成时宣称 v3 闭环完成。
...
v1/v2/v3 编写与归档没有修改代码、执行仿真或部署；仅审查实现、查证原始资料并形成讨论方案。所列新增实现与云容量验收仍待执行。v1、v2 原文均保留，当前实施讨论使用本文 v3；进入实施后按 F0 更新正式合同，不能把本方案当作已经生效的 API/schema。
