# Analog Canvas 仿真纵向闭环方案 v5

日期：2026-09-04。版本：v5。状态：讨论方案，供 Fable 交接及后续实施评审；所列新增工作尚未实现，尚未替换已接受 ADR。

本文是 v5 讨论方案。[v1 原文](simulation-vertical-integration-plan.md)、[v2 原文](simulation-vertical-integration-plan-v2.md)、[v3 原文](simulation-vertical-integration-plan-v3.md) 与 [v4 原文](simulation-vertical-integration-plan-v4.md) 独立保留、不覆盖；当前讨论以 v5 为准，已接受的产品行为仍以现行 spec 和 ADR 为准。
...
v5 只新增并展开前两项已确认方向，同时明确搁置 UI 信息架构。它不表示相应 API、MCP、schema 或界面已经实现。
...
v2 补充讨论时检查的主工作树为 `42f31aa5`。v3 的 Project / Cell / 云端管理定向审查基线为 main `a469fa68`（#548）；v5 对 Agent Project 管理与无图 raw 创作的定向审查也以该 main 状态为依据。本文在既有文档分支新增，不因此宣称对其他仿真模块完成了最新 main 全量审计。下列“当前”能力是相应审查记录，实施前重新确认是否已被其他工作改变。

目标 owner：Fable。代码实施另开 `codex/` 分支；本文件是 v5 讨论方案，不算已接受产品契约。
...
| 项目管理 | 私有 Cloud Project CRUD、revision guard、现有人类项目/Cell 操作、本地 Project 文件 | 可搜索的 Cell 选择能力、跨工程依赖闭包导入与本地实例化；不重建存储 |
...
2. Simulation 不建立第二套电路编辑协议或 controller。无论后续采用同页工作区、路由页面还是其他人类信息架构，DUT/TB 都复用现有画布编辑语义、wire、snap、撤销、层次导航、保存与 Agent transaction；具体入口、跳转和承载面不在 v5 冻结。
...
**当前基础与缺口。** 云端已经是私有 Project CRUD，不是二十个固定槽：每份有完整 Project JSON、当前 revision、名称、预览；20 是数量上限。现有服务不是可按旧 revision 下载的历史仓库。Project 内已经可以有多个 Cell，`childDocumentId` 只引用本工程定义，schema 校验引用、接口和层次环。Shelf 与 Cell Manager 是当前人类界面事实，但 v5 只要求复用其底层能力，不决定后续仍由哪个入口或页面承载。
...
**管理能力的最小范围。** 人类与 Agent 都必须能搜索工程、检查 Cell 的可复用接口/依赖/调用者，并执行放置或导入并放置；本地 Cell 改名、接口与调用关系仍由同一底层管理能力处理。不要把文件夹、标签、团队权限、历史版本、云配额扩大变成本次前置条件。页面入口、列表层级、跳转和布局全部留给 §7 后续讨论。
...
v5 **不冻结** Simulation 从何处进入、是否切换路由/工作区、进入后怎样跳转和排布，以及何时加载仿真服务。下面只列人类流程必须能够完成的能力，不构成页面布局、导航或 eager/lazy loading 合同；这些问题在下一轮单独讨论后再形成决定。

首用能力需要覆盖：从本工程或授权源工程选择 DUT → 检查接口与依赖（跨工程先预览并复制）→ 显式创建 TB 并放置默认 DUT → 继续放置其他 DUT/支持 Cell、源、负载并连接。默认一个 DUT 只是可选便捷起点；不能自动猜电源电压、输入幅度或负载，也不更改工程 top。

- Project/Cell 复用：搜索、检查接口/依赖、放置或导入；导入预览清楚区分本地副本与远程原件，正常 Editor 同样可用。
- 画布编辑：复用现有 canvas/controller，切换当前 TB document，保留所有常用编辑能力。
- 仿真设置：OP/AC/TRAN、频率或时间范围/步长、源 DC/AC/PULSE/SIN 参数、probe 清单、环境与温度；常用项优先，高级项可分级呈现。
- Pick Net：从现有 hit/resolver 得到对象地址，高亮选中对象，退出 pick 不产生电路选择/编辑副作用。
- 结果查看：OP 表/画布标注、AC 频响、TRAN 时间波形、CSV、日志及原始产物；具体停靠、标签与页面关系待 UI 讨论决定。
- 编排预览/导出：运行前即可查看结构网表、编排 deck 和依赖；复用 File/Netlist 的交付方式，运行后另有“这次运行原件”，不能用同一个含糊的 Netlist 按钮混淆来源。
- 进阶文本编辑：查看/编辑入口及源文件、显式接管、提交 raw 并保存/导出。无需先导入为图形；转换、未保存变化、缺依赖和回标能力都可见，不做任意 SPICE 与 Canvas 的自动双向同步。
...
- no binding/启动失败仍是环境不可用，不影响原 Editor/Gallery 使用；未来人类界面在取得 capabilities 后应准确呈现环境不可用，展示位置与加载时机留给 UI 讨论，Digital 保持关闭。
...
### F5：人类 Simulation 能力接入（信息架构待定）

Owner：apps/editor。依赖 F2-F4 API 稳定；不复制 App.tsx。入口、导航、布局和加载时机须先完成 v5 后续 UI 讨论，本方案不预定结论。

先接入 F1P/F1R 的 Project/Cell 共用服务与接口/依赖预览、同工程放置、跨工程导入并放置；正常 Editor 和 Simulation 共用，不引入第二套 Project manager。然后按后续冻结的信息架构接入 TB 画布能力、参数编辑、Net pick、OP/AC/TRAN 波形与 CSV、stale 提示与错误导航，以及正式编排预览/下载、进阶 raw 文件编辑/接管/提交和运行原件下载。

退出：用户可从两个工程导入 Cell 组成同一 TB、继续重复放置且知道它们是本地副本；工程默认入口保持不变；用户无需手写网表即可完成常用 OP/AC/TRAN；raw 人类编辑能力可完成 Agent 同等进阶操作而不要求每种语法都有表单；编辑期间旧结果不误贴，无映射 raw 仍能查看/导出数值；DUT/TB/Setup/作者文件保存重载；既有绘图与快捷键行为不受人类仿真界面接入破坏；生产 Digital 仍禁用。
...
F1R 在 F0 后独立推进 Cell 复用核心；F1P 在 F0 后形成 Project shared service，并在 F4/F5 接入前与 F1R 汇合。F2/F3 可先使用本工程 fixture，不依赖 Project 管理 UI。多 DUT/跨工程验收进入 F7，但不重建云端存储平台。

F6 的环境/部署验证在 F0 后并行，最终与 F3/F7 汇合。F2 的源/编排文件导出不等待 F3 跑通；F4 的 transport/workspace 可在 F0 的 RawInputBundle 合同冻结后并行，最终 simulation 接入仍依赖 F2/F3。先得到 Agent 可运行的真实 OP 垂直切片，再扩展 AC、TRAN、raw 输出与完整 GUI；三分析、两种输入和中间产物均是首期退出条件，不能在仅 OP 完成时宣称 v5 闭环完成。
...
v1/v2/v3/v4/v5 编写与归档没有修改代码、执行仿真或部署；仅审查实现、查证原始资料并形成讨论方案。所列新增实现与云容量验收仍待执行。v1、v2、v3、v4 原文均保留，当前实施讨论使用本文 v5；进入实施后按 F0 更新正式合同，不能把本方案当作已经生效的 API/schema。
