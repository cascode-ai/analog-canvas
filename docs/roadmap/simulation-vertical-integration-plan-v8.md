# Analog Canvas 仿真纵向闭环方案 v8

日期：2026-09-04。版本：v8。状态：讨论方案，供 Fable 交接及后续实施评审；所列新增工作尚未实现，尚未替换已接受 ADR。

本文是 v8 讨论方案。[v1 原文](simulation-vertical-integration-plan.md)、[v2 原文](simulation-vertical-integration-plan-v2.md)、[v3 原文](simulation-vertical-integration-plan-v3.md)、[v4 原文](simulation-vertical-integration-plan-v4.md)、[v5 原文](simulation-vertical-integration-plan-v5.md)、[v6 原文](simulation-vertical-integration-plan-v6.md) 与 [v7 原文](simulation-vertical-integration-plan-v7.md) 独立保留、不覆盖；当前讨论以 v8 为准，已接受的产品行为仍以现行 spec 和 ADR 为准。
...
v2 补充讨论时检查的主工作树为 `42f31aa5`。v3 的 Project / Cell / 云端管理定向审查基线为 main `a469fa68`（#548）；v5 对 Agent Project 管理与无图 raw 创作、v6 对 Cell Symbol/TB 衔接与人类 UI、v7 对 Cloudflare staging/deployment 的定向审查也以该 main 状态为依据。v8 的定向审查基线更新为 `origin/main` 的 `ad9d2725`（#566）：独立 public/noindex Preview、真实 Cloudflare ngspice Container、Run Metadata V1/environment fingerprint 与 signal/空输出失败分类已经落地。PR #559、#563 仍是 open proposal，Issue #560、#561、#564 仍是 task card，不写成当前已接受合同。下列“当前”能力是这次审查记录，实施前仍须重新确认。

目标 owner：Fable。代码实施另开 `codex/` 分支；本文件是 v8 讨论方案，不算已接受产品契约。

### 三项开放决策

#### D1：Cloud gate channel

当前 main 已按 ADR 0057 部署独立 Preview Worker、独立配置和真实 Container；legacy `env.staging` 仍存在但没有 Container，[PR #563](https://github.com/cascode-ai/analog-canvas/pull/563) 提议在 production 改为 release-only 后将其退役。最终采用现有 Preview、v7 的 private staging，还是更小的等价云验收面，本版不选择。必须只保留一个正式 cloud acceptance channel；若最终选择与 ADR 0057 不同，应通过正式 ADR 修订，而不是并存两套前置环境。

#### D2：Promotion evidence 与 workflow

已冻结的目标只有：production 不能推广一个无法可靠证明已经完成指定云验收的候选，候选或验收定义变化后旧证据不得继续有效。是否使用 receipt、最小字段是什么、是否 build once、如何处理允许的环境 overlay、手动恢复和人工 review，继续讨论；v8 不冻结 TypeScript interface、工作流步骤或审批频率。

#### D3：SKY130 模型环境

当前 Preview 使用的 binned 模型不能覆盖 [Issue #551](https://github.com/cascode-ai/analog-canvas/issues/551) 所示的宽器件参考电路，benchmark 使用另一套 continuous 模型环境。首版采用 binned、continuous、两者并存或另一份经验证 pack，本版不选择。F0 关闭前必须明确产品支持边界、来源与 digest、corner、simulator 配对、可执行 reference fixture 和资源成本；只写“pinned SKY130”不能算决定。
...
| 发布 | 独立 Preview 配置/Worker/Container 已由 main 自动部署；production 仍保留 legacy staging 前置流程 | D1 选定唯一云验收通道、D2 冻结 promotion evidence 后收敛发布流程；open PR 不提前当作现状 |
...
4. 当前 Preview route 可用于平台探针，但正式执行仍须接入授权、配额、每作业 cwd/scratch、输入输出上限与隔离。页面公开与计算接口匿名开放是两件事，D1 不应通过另建协议绕开同一 executor 安全边界。
...
- 仍欠缺 async Simulation Resource、structured/raw prepared input、正式 OP/AC/TRAN parser/result、鉴权/限额/隔离、实际 cancel、完整 artifact 留存以及 D1/D2/D3 决策后的 promotion。
- no binding/启动失败仍是环境不可用，不影响原 Editor/Gallery 或文件导出；capabilities 不应为了探测配置唤醒 Container，Digital 保持关闭。

### 8.3 Cloud acceptance 的不变量

localhost、单元测试和本地 Docker 不能证明 Cloudflare 的 Worker routing、binding、Durable Object、Container image、冷启动、实际模型路径和平台限额正确。无论 D1 最终选择哪一个云验收通道，都必须满足：
...
- 计算入口有明确准入、滥用控制与资源边界，CI 能以受控身份执行，公开页面不自动获得匿名无限算力；具体是否要求用户身份由 D1 决定；
- 同一候选验收期间不会被另一次部署悄悄覆盖；若是共享通道，至少有 deployment identity 与 workflow concurrency；
...
### 8.4 D1/D2/D3 与 production safety 的边界

D1 决定唯一 cloud acceptance channel 的拓扑、可见性和数据边界；D2 决定怎样以足够简单且可靠的 evidence 证明“被验收候选”就是“待推广候选”；D3 决定 hosted SKY130 环境及其能力边界。这三项在 F0 关闭前必须写入正式 ADR/spec/deployment 合同，但 v8 不预设其答案。

D2 无论选择 receipt、release record 或其他较小机制，都必须回答：候选如何标识，哪些变化使证据失效，Preview/production 允许哪些环境差异，重新构建是否可接受，手动恢复怎样不绕过验收。证据只属于 CI/release，不进入 Project、SimulationResult 或第四套产品 metadata；本版不冻结字段列表。

Cloudflare Container rollout 不是 Worker 与 image 的原子切换：Worker 可先激活，旧/新 image 可能并存，普通 Worker rollback 也不等于 Container target 已回退。因此 production simulation 首次启用或更新时，流程必须在公开流量启用前确认实际 runtime fingerprint/image/model 已收敛，并提供 Container 独立恢复；具体步骤与审批频率属于 D2 的简洁性/可靠性讨论。
...
F0 还必须关闭 §0 的 D1/D2/D3，但 v8 不替产品讨论选择答案：

- D1 写入唯一 cloud acceptance channel、数据/鉴权边界及与当前 ADR 0057 的关系。
- D2 写入足够简单且可靠的候选验收/promotion evidence、失效和恢复规则。
- D3 写入首期 hosted SKY130 环境集合/能力边界的来源、identity 与 acceptance fixture。

当前 Preview 的电阻分压和模型运行只作为已经取得的平台证据；它们不自动证明 D1/D2/D3 已决定，也不冒充 structured/raw、GUI/MCP 或正式结果闭环。F0 可以与不依赖三项答案的 F1/F1R/F1P/F2、F3 parser 和 F6 runtime hardening 并行，但 D1/D2/D3 未关闭时不得宣称 F6 promotion 或 F7 上线完成。

退出：上述产品合同与 D1/D2/D3 在现行 spec/ADR/deployment 文档中明确；open PR/Issue 的边界被有意吸收或修订；没有改变 DUT/Net、覆盖 raw 文本、复制 setup owner、文件运输或云验收协议的隐含路径。字段只做到首版所需，不为 sweep/优化预建通用工作流。
...
Owner：worker/containers/deploy workflow。F0 后可以先推进不依赖 D1/D2/D3 的 runtime hardening；D1 决定最终云验收面，D2 决定 promotion evidence/workflow，D3 决定模型环境。F6 实现这些已接受决定的机械边界，不替产品讨论选答案。

F6 负责真实 binding/DO/Container、实际实例规格、单槽/busy、timeout/process-tree kill、非 root 与每作业隔离、输入输出上限、cold/warm、health、runtime fingerprint、rollout convergence 和独立 Container recovery。当前 Preview 的 Worker、Container、`standard-2` 与平台 probe 是可复用基础，不重建第二套 route 或 executor。公开 raw `.control` 与 structured 输入都按不可信作业处理，不能靠只允许 helper 来规避隔离。

F6 不拥有 run 的数值完成语义和 parser（F3），不拥有 GUI/MCP 闭环（F4/F5），不签署某个候选是否通过（F7），也不选择 hosted model pack（D3）。D2 若保留跨环境 artifact/evidence，F6 只实现其构建、记录、失效、promotion 与恢复机制。

退出：D1 选定的云验收面能重复运行真实 Container；鉴权/配额、busy、timeout/cancel、进程树、作业隔离和 artifact 留存可验证；实际 runtime 规格、warm/cold、peak RSS、TRAN 输出体积和费用有证据；rollout 未收敛时不会公开启用新 simulation，Container target 可独立恢复。F3 的 API/numeric gate 可以在其上运行，D2 选定机制能可靠阻止未经匹配云验收的 promotion。关掉 simulation 不影响 Editor、Gallery 或本地文件导出。

### F7：垂直验收与上线

Owner：整合负责人。依赖 F3/F4/F5/F6。

F7 针对一个明确候选，在 D1 选定的真实 Cloud acceptance channel 执行平台、数值、协议、隔离、GUI/MCP 与跨工程完整矩阵，消费 F3 的正式结果和 F6 的 substrate，并形成 D2 最终选定的 promotion evidence。F7 不实现第二个 runner、parser、smoke API 或 receipt schema。localhost/local Docker 只作为更早的开发证据；production 不承担首次发现云绑定、Container、模型或结果协议错误的职责。
...
18. Cloud promotion：D1 选定且不能写 production 私有数据的云验收面运行自动平台/OP/AC/TRAN、D3 模型、raw 和失败门禁，GUI 与 MCP 各完成一条自动 smoke。F7 形成 D2 选定的候选 evidence；production 只能推广与该证据匹配的候选，并在公开 simulation 启用前确认 Container rollout/runtime identity 收敛，继续自己的 health 与独立 Worker/Container recovery。人工 review 是否需要及频率属于 D2，不由本条预设。
...
├─ close D1 / D2 / D3
├─ F1 / F1R / F1P → F2 → F3
├─ F6 runtime hardening（不依赖三项答案的部分可先做）
└─ F5-0 optional cloud probe（非关键路径、必须退役）

F3 + F6 chosen cloud surface
→ cloud API/numeric gate
→ F4 Agent/MCP + F5 human UI
→ cloud GUI/MCP vertical gate
→ F7 candidate acceptance
→ D2-selected promotion evidence / production promotion
```
...
F6 的 runtime hardening 可在 F0 后并行；D1/D2/D3 关闭后才把具体云验收与 promotion mechanics 接上。F3 结果合同就绪后形成自动数值/API gate，F4/F5 完成后再加入 MCP/GUI 云闭环；F5-0 可更早用于观察链路，但没有任何正式完成度含义。F2 的源/编排文件导出不等待 F3 跑通；F4 的 transport/workspace 可在 F0 的 RawInputBundle 合同冻结后并行，最终 simulation 接入仍依赖 F2/F3。先得到 Agent 可运行且在真实 Cloudflare 环境验证的 OP 垂直切片，再扩展 AC、TRAN、raw 输出与 v8 task UI；三分析、两种输入、中间产物、Current top→TB、按需 UI 与 cloud acceptance 均是首期退出条件，不能在仅 localhost 或 platform probe 完成时宣称 v8 闭环完成。

### 当前 GitHub 承接关系

| 条目 | 在 v8 中的定位 |
| --- | --- |
| [PR #559](https://github.com/cascode-ai/analog-canvas/pull/559) | F0 的 normative contract 候选；合入前更新到 v8，并显式关闭或引用 D1/D2/D3 |
| [Issue #560](https://github.com/cascode-ai/analog-canvas/issues/560) | F1 task card，方向与 v8 一致 |
...
| [Issue #551](https://github.com/cascode-ai/analog-canvas/issues/551) | D3 的模型覆盖证据，不在 v8 里预判方案 |
...
| [PR #563](https://github.com/cascode-ai/analog-canvas/pull/563) | D1/D2 的 release-channel 候选，不算当前 main 已生效结论 |
...
- 第二个 Editor App、Simulation 专用 route、常驻三栏/多宫格 dashboard；v8 只增加同一 Editor 内的临时 task bar、按需交互与结果 drawer。
- 同时长期维护 Preview 与 private staging 两套正式云验收通道、每个 PR 一套 Cloudflare Worker/DO/Container、复制 production 私有数据到验收环境或跨环境共享 production Durable Object namespace。D1 只选择一个正式通道；D2 决定候选 promotion evidence，不在本条暗定 artifact/receipt 机制。
...
v1/v2/v3/v4/v5/v6/v7/v8 编写与归档没有修改代码、执行仿真或部署；仅审查实现、GitHub 上的当前任务与现有云端证据并形成讨论方案。v1、v2、v3、v4、v5、v6、v7 原文均保留，当前实施讨论使用本文 v8。D1/D2/D3 仍是开放决策；open PR/Issue 不因此成为已接受 API/schema/deployment 合同，进入实施后仍须按 F0 更新正式 spec/ADR。
