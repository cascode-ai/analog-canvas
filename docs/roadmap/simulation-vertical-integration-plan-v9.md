# Analog Canvas 仿真纵向闭环方案 v9

日期：2026-09-04。版本：v9。状态：讨论方案，供 Fable 交接及后续实施评审；D1/D2 已获产品确认，D3 仍待产品讨论；所列新增工作尚未实现，尚未替换已接受 ADR。

本文是 v9 讨论方案。[v1 原文](simulation-vertical-integration-plan.md)、[v2 原文](simulation-vertical-integration-plan-v2.md)、[v3 原文](simulation-vertical-integration-plan-v3.md)、[v4 原文](simulation-vertical-integration-plan-v4.md)、[v5 原文](simulation-vertical-integration-plan-v5.md)、[v6 原文](simulation-vertical-integration-plan-v6.md)、[v7 原文](simulation-vertical-integration-plan-v7.md) 与 [v8 原文](simulation-vertical-integration-plan-v8.md) 独立保留、不覆盖；当前讨论以 v9 为准，已接受的产品行为仍以现行 spec 和 ADR 为准。
...
v2 补充讨论时检查的主工作树为 `42f31aa5`。v3 的 Project / Cell / 云端管理定向审查基线为 main `a469fa68`（#548）；v5 对 Agent Project 管理与无图 raw 创作、v6 对 Cell Symbol/TB 衔接与人类 UI、v7 对 Cloudflare staging/deployment 的定向审查也以该 main 状态为依据。v8 的定向审查基线更新为 `origin/main` 的 `ad9d2725`（#566）：独立 public/noindex Preview、真实 Cloudflare ngspice Container、Run Metadata V1/environment fingerprint 与 signal/空输出失败分类已经落地。v9 在同一最新 `origin/main` 上冻结 D1/D2 的产品方向；PR #559、#563 仍是 open implementation proposal，Issue #560、#561、#564 仍是 task card，产品确认不等于代码已经合入。下列“当前”能力是这次审查记录，实施前仍须重新确认。

目标 owner：Fable。代码实施另开 `codex/` 分支；本文件记录产品讨论结果，但 D1/D2 仍需同步到正式 ADR/spec/deployment 并实现，D3 仍未关闭。
...
Production release/tag 或手动入口必须解析到完整 SHA，并只接受该 SHA 的匹配成功 Preview run；部署后继续校验自己的 build identity、health 和实际 runtime fingerprint。Preview 成功不替代 Production 的轻量 smoke、Container rollout 收敛与独立恢复。是否 build once、允许哪些配置 overlay、是否增加人工审批及具体 rollout 命令不在 v9 写死；若选择重建，实际身份仍必须与已接受环境合同匹配。事故恢复可以使用仓库既有 production-incident 例外，但必须留下书面记录，普通 release 不得静默绕过门禁。

#### D3：SKY130 模型环境

当前 Preview 使用的 binned 模型不能覆盖 [Issue #551](https://github.com/cascode-ai/analog-canvas/issues/551) 所示的宽器件参考电路，benchmark 使用另一套 continuous 模型环境。首版采用 binned、continuous、两者并存或另一份经验证 pack，v9 仍不选择。F0 关闭前必须明确产品支持边界、来源与 digest、corner、simulator 配对、可执行 reference fixture 和资源成本；只写“pinned SKY130”不能算决定。
...
| 执行 | Worker route、Docker harness、local-host、60 秒默认/120 秒上限、Preview Worker 的 `standard-2` 单实例 Container、环境 metadata；signal-killed/无输出不再算完成 | 正式 async run/result、鉴权与配额、busy guard、非 root/每作业隔离、进程树终止、原始数值收集与 rollout 收敛证据 |
...
| 发布 | 独立 Preview 配置/Worker/Container 已由 main 自动部署；production 仍保留 legacy staging 前置流程 | 实施 D1 的 release-only/legacy staging retirement，并按 D2 补齐 build identity、exact-SHA Preview gate、environment evidence 与 production recovery；D3 仍待冻结 |
...
5. 环境必须有可验证 lock：基础镜像、ngspice binary、模型来源与内容 hash、runtime image digest。具体模型 pack 由 D3 决定；在该决定前只能报告 observed metadata，不能宣称产品环境已经 pinned。
...
- 仍欠缺 async Simulation Resource、structured/raw prepared input、正式 OP/AC/TRAN parser/result、鉴权/限额/隔离、实际 cancel、完整 artifact 留存、D1/D2 的发布机械实现以及 D3 模型决定后的完整 promotion。
...
1. Worker shell、capabilities、DO binding/migration、Container health、cold/warm start、实际实例规格与 environment fingerprint。
2. 电阻分压 OP、RC AC、RC TRAN，以及 D3 选定模型环境中的非线性器件 fixture；数值、单位、轴和容差与固定 ngspice reference 对照，不能只看 HTTP 200 或进程退出码。
...
### 8.4 已接受的 Cloud gate/promotion 合同与开放的 D3
...
Preview gate 必须在验收前后确认 `buildSha === expected full SHA`，并记录 opaque `environmentFingerprint`。Production promotion 只接受该 SHA 最新已完成且成功的 required Preview run；较新的 failed/cancelled run 会阻止普通 promotion，直到重新验收成功。Production checkout/deploy 同一 full SHA，并在上线后验证自己的 build identity、health 与实际 runtime fingerprint。v9 不要求 build-once、完整 Worker bundle 字节相同、巨型 manifest/fixture hash 清单或固定人工审批频率；若选择重建，锁定依赖与运行时身份检查必须证明实际环境仍符合合同。
...
D3 仍决定 hosted SKY130 环境及其能力边界。在它关闭前，Preview 的 binned platform probe 不能被描述为正式产品模型环境，模型相关 F7 gate 与 production simulation 上线仍未完成。
...
- D3 写入首期 hosted SKY130 环境集合/能力边界的来源、identity 与 acceptance fixture。

当前 Preview 的电阻分压和模型运行只作为已经取得的平台证据；它们不自动证明 D1/D2 已实现，也不冒充 structured/raw、GUI/MCP 或正式结果闭环。D1/D2 mechanics、F1/F1R/F1P/F2、F3 parser 和 F6 runtime hardening 可以立即并行；D3 未关闭时不得宣称模型相关 F7 gate、F6 promotion 或 production simulation 上线完成。

退出：上述产品合同、已决定的 D1/D2 与最终 D3 在现行 spec/ADR/deployment 文档中明确；open PR/Issue 的边界被有意吸收或修订；没有改变 DUT/Net、覆盖 raw 文本、复制 setup owner、文件运输或云验收协议的隐含路径。字段只做到首版所需，不为 sweep/优化预建通用工作流。
...
Owner：worker/containers/deploy workflow。D1/D2 已决定，相关 mechanics 可以立即推进；D3 仍决定模型环境。F6 复用现有 Preview 作为唯一验收面，不新建 staging，并实现受保护计算、候选身份、promotion 与恢复边界，不替产品讨论选择 hosted model pack。

F6 负责真实 binding/DO/Container、实际实例规格、单槽/busy、timeout/process-tree kill、非 root 与每作业隔离、输入输出上限、cold/warm、health、runtime fingerprint、rollout convergence 和独立 Container recovery。当前 Preview 的 Worker、Container、`standard-2` 与平台 probe 是可复用基础，不重建第二套 route 或 executor。Preview 的 simulation execution 加入 CI service identity、授权测试者准入和配额；公开 raw `.control` 与 structured 输入都按不可信作业处理，不能靠只允许 helper 来规避隔离。release-only Production 稳定后退役 legacy `env.staging`、gate、workflow job 与残留 Worker。

F6 不拥有 run 的数值完成语义和 parser（F3），不拥有 GUI/MCP 闭环（F4/F5），不签署某个候选是否通过（F7），也不选择 hosted model pack（D3）。它注入完整 build SHA、提供只读 build identity、在 Preview gate 前后核对该 identity、记录 opaque environment fingerprint，并让 Production 只消费 exact SHA 最新已完成且成功的 required Preview run。GitHub run 是 receipt；F6 只在必要时附小型 machine-readable output，不建立 receipt 数据库或产品协议。
...
4. D3 选定模型环境中的 reviewed SKY130 电路：OTA 的 OP+AC，以及 inverter 或合适 OTA TB 的脉冲 TRAN，同模型/corner/温度/激励/负载与直接 ngspice 对照，覆盖非线性器件及该 profile 声明支持范围的边界；具体 pack 和宽器件 fixture 在 D3 决定后冻结。
...
10. 缺模型、probe 消失、AC/TRAN 配置非法、收敛失败、dropped-input、signal/OOM、空/截断/必需 vector 缺失、未支持格式输出、busy、timeout、cancel、产物过期、资源超限与路径越界；并确认云端实际实例规格、cold/warm 行为和 rollout 后 runtime identity。
...
18. Cloud promotion：不能写 production 私有数据且 execution 受保护的 Preview 运行自动平台/OP/AC/TRAN、D3 模型、raw 和失败门禁，GUI 与 MCP 各完成一条自动 smoke。gate 前后 `buildSha` 都等于 expected full SHA，并记录 opaque `environmentFingerprint`；所有 required jobs 成功后该 GitHub run 成为 receipt。Production 只推广同一 SHA 最新已完成且成功的 required run，部署后确认自己的 build/runtime identity、Container rollout 与 health，并保留独立 Worker/Container recovery。v9 不要求固定人工 review 频率。
...
```text
F0
├─ formalize accepted D1 / D2 + close D3
├─ F1 / F1R / F1P → F2 → F3
...
F6 的 Preview 鉴权/runtime hardening、build identity 与 exact-SHA workflow gate 可立即并行；D3 只阻塞模型 profile 相关 acceptance 与最终 production simulation 上线。F3 结果合同就绪后形成自动数值/API gate，F4/F5 完成后再加入 MCP/GUI 云闭环；F5-0 可更早用于观察链路，但没有任何正式完成度含义。F2 的源/编排文件导出不等待 F3 跑通；F4 的 transport/workspace 可在 F0 的 RawInputBundle 合同冻结后并行，最终 simulation 接入仍依赖 F2/F3。先得到 Agent 可运行且在 Preview 验证的 OP 垂直切片，再扩展 AC、TRAN、raw 输出与 v9 task UI；三分析、两种输入、中间产物、Current top→TB、按需 UI 与 cloud acceptance 均是首期退出条件，不能在仅 localhost 或 platform probe 完成时宣称 v9 闭环完成。

### 当前 GitHub 承接关系

| 条目 | 在 v9 中的定位 |
| --- | --- |
| [PR #559](https://github.com/cascode-ai/analog-canvas/pull/559) | F0 的 normative contract 候选；合入前更新到 v9，写入已决定的 D1/D2，并保持 D3 为明确的产品待决项 |
| [Issue #560](https://github.com/cascode-ai/analog-canvas/issues/560) | F1 task card，方向与 v9 一致 |
...
| [Issue #551](https://github.com/cascode-ai/analog-canvas/issues/551) | D3 的模型覆盖证据，不在 v9 里预判方案 |
...
- 第二个 Editor App、Simulation 专用 route、常驻三栏/多宫格 dashboard；v9 只增加同一 Editor 内的临时 task bar、按需交互与结果 drawer。
...
v1/v2/v3/v4/v5/v6/v7/v8/v9 编写与归档没有修改代码、执行仿真或部署；仅审查实现、GitHub 上的当前任务与现有云端证据并形成讨论方案。v1、v2、v3、v4、v5、v6、v7、v8 原文均保留，当前实施讨论使用本文 v9。D1/D2 已由产品讨论接受，但只有同步到正式 spec/ADR/deployment 并实现后才成为仓库行为；D3 仍是开放产品决策。open PR/Issue 不因被引用而自动成为已接受 API/schema/deployment 合同。
