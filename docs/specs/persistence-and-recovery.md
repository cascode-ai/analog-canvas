...
Portable Projects use canonical schema-25 `.icproj.json`. The current-only
...
platform supports it. Schema 24 upgrades to 25 at ingestion; serialization
always writes schema 25. Older schemas are rejected outside the rolling
current-and-previous compatibility window.

Recovery state is a non-authoritative browser safety copy. It may restore a
complete schema-25 Project or a schema-24 record that validates after the
