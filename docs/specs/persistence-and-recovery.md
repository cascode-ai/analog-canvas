...
Project content uses canonical schema-30 JSON. A private Cloud Project is the
...
before import or Cloud Save. Schema 29 upgrades to 30 at ingestion; serialization
always writes schema 30. Older schemas are rejected outside the rolling
current-and-previous compatibility window.

Recovery state is a non-authoritative browser safety copy. It may restore a
complete schema-30 Project or a schema-29 record that validates after the
