...
geometric, and presentation facts. The current model is strict schema 53 and has
...
Persistence writes only schema 53. The reader carries every schema in its
explicit 24→53 upgrade chain forward, then supplies the current model only; no
