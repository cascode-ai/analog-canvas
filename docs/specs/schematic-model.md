...
geometric, and presentation facts. The current model is strict schema 54 and has
...
Persistence writes only schema 54. The reader carries every schema in its
explicit 24→54 upgrade chain forward, then supplies the current model only; no
