...
export interface PrintedSpiceInstance {
  documentId: string;
  instanceId: string;
  startOffset: number;
  endOffset: number;
...
export interface PrintedSpiceParameter {
  documentId: string;
  instanceId: string;
  parameter: string;
  rawValue: string;
...
function renderSpice(
  ...
  const parameters: PrintedSpiceParameter[] = [];
  const instances: PrintedSpiceInstance[] = [];
  ...
  const globals = includeGroundGlobal
    ? ir.globals
    : ir.globals.filter((name) => name !== "0");
...
export function printSpectreNetlist(
  ir: DesignNetlistIR,
  includeGroundGlobal = false,
): string {
  const lines = [
...
export function printDesignNetlist(
  ...
  const hasVdd = ir.globals.some(
    (name) => name.replace(/!$/u, "").toLowerCase() === "vdd",
  );
  const globals = [
    ...new Set(["0", ...(hasVdd ? [] : ["VDD"]), ...ir.globals]),
  ];
  const exportIr = { ...ir, globals };
  return format === "spice"
    ? {
        extension: ".spi",
        mediaType: "application/x-spice",
        text: renderSpice(exportIr, false, false, true).text,
      }
    : {
        extension: ".scs",
        mediaType: "application/x-spectre",
        text: printSpectreNetlist(exportIr, true),
      };
}
